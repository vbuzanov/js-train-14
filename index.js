...
function getDayOfWeek(date) {
  ...
  if (typeof date.getTime === "function") {
    ...
    return { inputDate: inputDate, dayOfWeek: daysOfWeek[day] };
  } else {
    return "Помилка: вхідне значення має бути об'єктом Date";
  }
}
...
function getDaysInMonth(date) {
  ...
  if (typeof date.getTime === "function") {
    ...
    const dateLastDate = new Date(year, month + 1, 0);
    return { inputDate: inputDate, daysInMonth: dateLastDate.getDate() };
  } else {
    return "Помилка: вхідне значення має бути об'єктом Date";
  }
}
...
function getAge(birthDate) {
  ...
  if (typeof birthDate.getTime === "function") {
    const date = new Date();
    let diff = date.getFullYear() - birthDate.getFullYear();
    ...
    return { birthDate: "1990-05-15T00:00:00.000Z", age: diff };
  } else {
    return "Помилка: вхідне значення має бути об'єктом Date";
  }
}
...
//Виведе { birthDate: birthDateISO, age: 33 }
